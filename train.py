...
parser.add_argument("--tensorboard_log_dir", default=None, type=str, help="Tensorboard log directory.")
...
parser.add_argument("--train_img_mean", default=None, type=int, help="image mean value for training")
parser.add_argument("--num_workers", default=4, type=int, help="Number of worker for dataloader.")
parser.add_argument("--optimizer", default='SGD', type=int, help="Optimizer for training.")
parser.add_argument("--learning-rate", default=2.5e-4, type=float, help='Optimizer learning rate.')
...
parser.add_argument("--input_size_source", default=None)
parser.add_argument("--max_iters", default=50000, type=int, help="Maximum number of iterations for training.")
...
def main():
    ...
    if args.backbone_model == 'DeepLabv2':
        ...
        saved_state_dict = torch.load(args.model_dir, map_location=torch.device("cpu"))
        if 'DeepLab_resnet_pretrained' in args.model_dir:
            new_params = model.state_dict().copy()
            for i in saved_state_dict:
                i_parts = i.split('.')
                if not i_parts[i] == 'layer5':
